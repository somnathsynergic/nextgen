...
function CompositeSearch({ data, onSubmit, onReset, flag, totProjCount, totVendCount, totProdCount,loadProjects,loadVendors,loadProducts }) {
  ...
  const [eventVendor,setEventVendor] = useState(false);
  const [eventProject,setEventProject] = useState(false);
          ...
          <div className="flex justify-between gap-4 my-4">
            ...
            <div>
              ...
              >
               ...
               {/* <SearchResult
                        totCount={totVendCount}
                        className={'w-[200px] border-2 bg-gray-200 border-green-900'} show={showVendor} event={eventVendor}
                        onHandleScroll={() => {
                          loadVendors(0,data.set_one.length + 10)
                      
                        }}
                        onPress={(e) => { setOne(e.name);
                            setOneCode(e.code);
                            setShowVendor(false);
                      ...
                      .filter((e) => e.name?.toLowerCase().includes(set_one_val.toLowerCase()))
                          ...
                          // data.set_two?.filter((e) => e.name?.toLowerCase().includes(project?.toLowerCase()) || e.proj_id?.toLowerCase().includes(project?.toLowerCase()))
                        }
                      /> */}
            </div>
            <div>
              <TDInputTemplate
                ...
                // handleFocus={e => {setShow(true);setEvent(e)}}
              ...
              >
                ...
                <ul class=" divide-y max-h-32 overflow-y-scroll mt-2 divide-gray-200 dark:divide-gray-700">
                  ...
                  {data?.set_two.filter((e) =>
                    ...
                    {/* <SearchResult
                        totCount={totProjCount}
                        className={'w-[200px] border-2 bg-gray-200 border-green-900'} show={show} event={event}
                        onHandleScroll={() => {
                          loadProjects(0,data.set_two.length + 10)
                      
                        }}
                        onPress={(e) => { setTwo(e.name);
                            setTwoCode(e.code);
                            setShow(false);
          ...
          <div className={"flex justify-between gap-4"}>
            ...
            <div>
              ...
              >
                ...
                {/* <SearchResult
                        totCount={totProdCount}
                        className={'w-[200px] border-2 bg-gray-200 border-green-900'} show={showItem} event={eventItem}
                        onHandleScroll={() => {
                          loadVendors(0,data.set_four.length + 10)
                      
                        }}
                        onPress={(e) => { setFour(e.name);
                            setFourCode(e.code);
                            setShowItem(false);
