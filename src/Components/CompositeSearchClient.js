...
function CompositeSearchClient({data, onSubmit, onReset,flag}) {
    ...
    const [set_nine_val, setNine] = useState("");
        ...
        >
          ...
          <div onClick={handleClickInside} className="gap-4 mt-5">
            ...
            <div className="flex justify-between gap-4 my-4">
                ...
                <div>
                <TDInputTemplate
                  placeholder={`Type ${data?.set_nine_lbl}`}
                  type="date"
                  label={data?.set_nine_lbl}
                  name={"set_nine_val"}
                  formControlName={set_nine_val}
                  handleChange={(val) => {
                    setNine(val.target.value);
                    console.log(val.target.value);
                  }}
                  min={set_eight_val}
                  disabled={!set_eight_val}
                  mode={1}
                  // data={data?.set_one}
                />
                 {set_eight_val && !set_nine_val && <VError title={'Required'}/>}
                </div>
            </div>
            <div className="flex justify-between gap-4 my-4">
              <div>
...
export default CompositeSearchClient
