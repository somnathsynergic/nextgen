...
function DrawerComp({open,onClose,flag,data}) {
  ...
  return (
    <Drawer keyboard={loading?false:true}
    headerStyle={{backgroundColor:'#014737',color:'white',fontWeight:'bold'}}
    
    headerClassName={'bg-green-900 text-white'}  maskClosable={loading?false:true} closeIcon={loading?false:
    <div className='bg-white rounded-full p-0.5 float-end'>
    <CloseOutlined style={{  color: '#014737' }} /></div>} width={flag!=7 && flag!=8?900:300} title={flag==1?"Add Vendor":flag==2?"Add Project":flag==3?"Add Item":flag==4?"Add Client":flag==5?"Add Unit":flag==6?"Add GST":flag==7?'Upload/View Vendor Receipt (PO: '+data.po+')':flag==8?'Upload/View Vendor MDCC (PO: '+data.po+')':'Reset Password'} onClose={onClose} open={open}>
