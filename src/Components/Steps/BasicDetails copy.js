...
import { ArrowRightOutlined, LoadingOutlined, PlusCircleOutlined, SyncOutlined } from "@ant-design/icons";
            ...
            <div className="sm:col-span-6">
              ...
              {vendor_name && <Viewdetails click={()=>{
                ...
                // axios.post(url+'/api/getvendorpoc',{id:+data?.vendor_name?+data?.vendor_name:+localStorage.getItem('vendor_name')}).then((resPoc)=>{
                  ...
                  console.log(resPoc)
                  setPocList(resPoc?.data?.msg)
                  axios.post(url+'/api/getvendordealsinfo',{id:localStorage.getItem('vendor_name')}).then(resDeals=>{
                    console.log(resDeals)
                    setDeals(resDeals?.data?.msg)
                    axios.post(url+'/api/getvendorbank',{id:localStorage.getItem('vendor_name')}).then(resBank=>{
                      setBank(resBank?.data?.msg)
                    setLoading(false)
