...
const numberToWords = (num) => {
    ...
    if (Number(whole) === 0) {
      ...
      result = inWords(Number(whole));
    }
  
    if (decimal) {
      const decimalWords = decimal.split('').map(d => a[Number(d)]).join(' ');
...
<div className="relative overflow-x-auto">
    <table className="w-full tabled-fixed border-collapse border border-gray-300 text-sm text-left rtl:text-right text-gray-700 dark:text-gray-400 ">
        ...
        <tbody>
         ...
         <td className="px-1 py-1  text-[9px] w-auto text-center border border-gray-300" rowSpan={2}>
                    {index+1}
                </td>
                <td className=" w-auto py-1 px-1 text-xs text-green-700 font-bold flex flex-col gap-1 text-wrap text-sm text-gray-900 whitespace-nowrap dark:text-white">
                    {item.prod_name}
                </td>
                <td className="  py-1 px-1 w-[2%] text-[8px]  text-center border  text-nowrap border-gray-300" rowSpan={2}>
                    {item.quantity} ({item.unit_name})
                </td>
                <td className="  py-1 px-1 w-[2%] text-[8px] text-center border border-gray-300" rowSpan={2}>
                {parseFloat(item.item_rt).toFixed(2)}
                </td>
                <td className=" py-1 px-1 w-[2%] text-[8px] text-center border border-gray-300" rowSpan={2}>
                    {item.discount} {item.discount?'('+parseFloat(item.discount_percent)?.toFixed(2)+'%)':''}
                </td>
                <td className=" py-1 px-1 w-[2%] text-[8px] text-center border text-wrap border-gray-300" rowSpan={2}>
                ...
                {/* <td className=" py-1 px-1 w-[2%] text-[8px] text-center text-nowrap border border-gray-300 " rowSpan={2}>
                ...
                */}
                ...
                <td className=" py-1 px-1 w-[2%]  text-[7px] text-center border border-gray-300" rowSpan={2}>
                    {
        ...
        {/* <tfoot> */}
            <tr class="font-semibold text-gray-900 dark:text-white py-3">
                ...
                <th class=" pb-3 py-1 px-4 text-[12px]  font-bold text-wrap text-green-700 text-nowrap" colspan={3}>{formatNumber(grandTot,grandTotUnit=='I'?'INR':grandTotUnit=='U'?'USD':'EUR')} ({numberToWords(grandTot)} {grandTotUnit=='I'?'Rupees':grandTotUnit=='U'?'Dollars':'Euros'} only)
                {/* item?.currency=='I'?'₹':item?.currency=='U'?'$':'€':'' */}
