...
function ClientDeliveryView() {
    ...
    useEffect(() => {
      ...
      axios.post(url + "/api/getpofordelivery", { id: 0 }).then((res) => {
        ...
        //   setCopy(res?.data?.msg.filter(e=>e.po_status=='A' && e.created_by==localStorage.getItem('email') && e.ware_house_flag=='N'));
        //   setPoData(res?.data?.msg.filter(e=>e.po_status=='A' &&  e.created_by==localStorage.getItem('email') && e.ware_house_flag=='N'));
        //   }
        //   if(localStorage.getItem('user_type')=='5'){
            setCopy(res?.data?.msg.filter(e=>e.po_status=='A' && e.ware_house_flag=='N' && e.created_by==localStorage.getItem('email')));
            setPoData(res?.data?.msg.filter(e=>e.po_status=='A' && e.ware_house_flag=='N' && e.created_by==localStorage.getItem('email')));
            // }
      });
    }, []);
...
export default ClientDeliveryView
