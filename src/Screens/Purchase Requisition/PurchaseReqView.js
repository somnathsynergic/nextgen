...
function PurchaseReqView() {
    ...
    const onAdvSearch = (val1, val2,val3,val4,val5,val6,val7) => {
      ...
      //setValue(0);
      // setVisible(true)
      axios.post(url+'/api/advanced_search_purchase_req',{vendor_id:val1,project_id:val2,part_no:val3,prod_id:val4,from_dt:val5,to_dt:val6,make:val7}).then(res=>{
        console.log(res)
    ...
    useEffect(() => {
      ...
      setLoading(true)
      axios.post(url + "/api/get_purchase_req", { id:0 }).then((res) => {
        console.log(res);
        setLoading(false);
    ...
    return (
      <>
        ...
        {copy.length == 0 && loading == false && (
         ...
         <div 
               ref={contentRef} className={isPrinting?"":"p-3"}>
              ...
              <table class={isPrinting?"w-full text-sm text-left rtl:text-right shadow-lg text-green-900dark:text-gray-400":"w-full mt-3 text-sm text-left rtl:text-right shadow-lg text-green-900dark:text-gray-400"}>
                <thead class={isPrinting?" text-md  text-gray-700 capitalize   bg-[#C4F1BE] dark:bg-gray-700 dark:text-gray-400":"text-md text-white capitalize mt-5  bg-green-500 dark:bg-gray-700 dark:text-gray-400"}>
                  <tr>
                   ...
                   {isPrinting && <th scope="col" class="p-4 w-1/5">
                      Action
                    </th>}
                  </tr>
                </thead>
