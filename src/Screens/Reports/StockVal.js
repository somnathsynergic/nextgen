            ...
            >
              <form>
                <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
                  ...
                  {type == "P" && <div className="sm:col-span-1">
                  ...
                  <div className={type == 'P' ? "sm:col-span-2" : "sm:col-span-1"}>
                    <TDInputTemplate
                      ...
                      handleFocus={e => op_prod.current.show(e)}
                      handleChange={(txt) => {
                        console.log(txt);
                        setProdVal(txt.target.value);
                        if (txt.target.value.length)
                          op_prod.current.show(txt)
                        else {
                          op_prod.current.hide(txt)
                          setProdCode()
                        }
                        // setLoading(true);
                        // getItemDetails(txt.target.value);
                    ...
                    <OverlayPanel ref={op_prod} className={type != 'W' ? "w-[980px] border-2 bg-gray-50 border-[#C4F1BE]" : "w-[480px] border-2 bg-gray-50 border-[#C4F1BE]"}>
                      <span className='text-xs text-green-900 italic'>Search results for: "{prodVal}"</span>
                      <ul class=" divide-y max-h-48 overflow-y-scroll mt-2 divide-gray-200 dark:divide-gray-700">
                        {prodList?.filter(e => e.name?.toLowerCase().includes(prodVal?.toLowerCase()) || e.part_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.part_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.model_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.article_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.make?.toLowerCase().includes(prodVal?.toLowerCase())).length > 0 && prodList?.filter(e => e.name?.toLowerCase().includes(prodVal?.toLowerCase()) || e.part_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.part_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.model_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.article_no?.toLowerCase().includes(prodVal?.toLowerCase()) || e.make?.toLowerCase().includes(prodVal?.toLowerCase()))?.map(lst => <li onClick={(e) => { op_prod.current.hide(e); setProdVal(lst.name); setProdCode(lst.code) }} class="pb-3 cursor-pointer  hover:bg-[#C4F1BE] group active:bg-green-900 rounded-md hover:duration-300 sm:py-1.5">
                          <div class="flex items-center rtl:space-x-reverse">
                  ...
                  <div className="sm:col-span-1">
                    <TDInputTemplate
                      placeholder="Stock in To"
                      type="date"
                      label="To"
                      name="dt"
                      formControlName={to_dt}
                      handleChange={(txt) => setToDt(txt.target.value)}
                      mode={1}
                      min={formatDate(
                        new Date(
                          new Date().setFullYear(new Date().getFullYear() - 3)
