...
function ApproveView() {
  ...
  const [item_lst, setItemList] = useState([]);
  const [item_lst_appr, setItemListAppr] = useState([]);
  const [labels, setLabels] = useState();
  const [visible, setVisible] = useState(false);
  const [trans_no, setTransNo] = useState("");
  const [from_proj_id, setFromProjId] = useState(0);
  const [to_proj_id, setToProjId] = useState(0);
  const [from_proj_name, setFromProjName] = useState("");
  const [to_proj_name, setToProjName] = useState("");
  const [flag, setFlag] = useState(0);
  const [to_projid,setTo] = useState("")
  const [from_projid,setFrom] = useState("")
  ...
  const getItemDetails = (item) => {
    console.log(item);
    setTransNo(item.trans_no);
    setFromProjId(item.from_proj_id);
    setFromProjName(item.from_proj_name);
    setToProjId(item.to_proj_id);
    setToProjName(item.proj_name||item?.to_proj_name);
    setTo(item.to_projid || "")
    setFrom(item.from_projid)
    setLoading(true);
    axios
      .post(url + "/api/get_transfer_stock_items", { trans_no: item.trans_no })
      .then((res) => {
        console.log(res);
        setLoading(false);
        setFlag(32);
        setItemList(res?.data?.msg);
        item_lst_appr.length = 0;
        res?.data?.msg?.map((item) => {
          item_lst_appr.push({
            ...
            check: item.approve_flag == "A" ? true : false,
            approved_qty:item.approved_qty,
            cancelled_qty:item.cancelled_qty,
            error:0,
            balance:item.balance,
            balance_copy:item.balance,
  ...
  const onChange = (e) => {
    ...
    if (e == 1) {
      setPoData(
        copy.filter((e) => e.approve_flag == "A" || e.approve_flag == "R")
      );
      console.log(po_data);
    } else if (e == 2) {
      setPoData(copy.filter((e) => e.approve_flag == "P"));
      console.log(po_data);
    }
    // else{
    //   setPoData(
  ...
  const onAdvSearch = (val1, val2, val3, val4, val5, val6, val7) => {
    ...
    // setVisible(true)
    axios
      .post(url + "/api/advanced_search_transfer", {
        from_proj_id: val1,
        to_proj_id: val2,
        part_no: val3,
        prod_id: val4,
          ...
          <div class="w-full relative flex justify-normal">
            <div class="flex items-center justify-between w-11/12">
              <motion.h2
                ...
                class="text-md w-48 capitalize text-nowrap font-bold text-white dark:text-white sm:block hidden mx-5"
              >
                {"Approve Transfer"}
              </motion.h2>
      ...
      <DialogBox
        ...
        data={
          flag == 33
            ...
            : {
                info: item_lst_appr,
                from_proj_name: from_proj_name,
                to_proj_name: to_proj_name,
                to_projid:to_projid,
                from_projid:from_projid,
                trans_no: trans_no,
              }
        }
        onPress={() => setVisible(false)}
        mode="A"
        onCloseApprove={(values) => getItemDetails(values)}
        onApprove={(status,values) => {
          {
            console.log(status,values);

            setLoading(true);
            axios
              .post(url + "/api/approve_transfer_items", {
                items: values,
                status:status,
                trans_no: trans_no,
