...
import { Button, Divider, Empty, Popover, Tag, Tooltip } from "antd";
import {
  LoadingOutlined,
  MinusOutlined,
  PlusOutlined,
...
function PtoWForm() {
    ...
    useEffect(() => {
      ...
      if (params.id > 0) {
        setLoading(true);
        axios.post(url + "/api/get_tpw_record", { id: params.id }).then((res) => {
          console.log(res);
          setLoading(false);
    ...
    const onSubmit = () => {
      setLoading(true);
      axios
        .post(url + "/api/save_transfer_ptow", {
          sl_no: +params.id,
                  ...
                  {itemDtls.map((item, index) => (
                    <>
                      ...
                      <div className={"sm:col-span-5 border-2"}>
                        <TDInputTemplate
                          placeholder="Quantity (should be >0 and <= project stock)"
                          type="number"
                          label=" "
                          name="qty"
