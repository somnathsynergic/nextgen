...
const [isPrinting, setIsPrinting] = useState(true);
  ...
  useEffect(() => {
    ...
    // });
    axios.post(url + "/api/get_twp", { id: 0 }).then((res) => {
      console.log(res);
      setLoading(false);
  ...
  const onAdvSearch = (val1, val2, val3, val4, val5, val6, val7,val8) => {
    ...
    axios
      ...
      .then((res) => {
        console.log(res);
        setAdvList(res?.data?.msg.filter(e=>e.trans_no.toString().includes('TWP')));
  ...
  return (
    <>
      <div className="flex items-center  justify-end h-14 -mt-[72px] w-auto dark:bg-[#22543d] md:flex-row space-y-3 md:space-y-0 rounded-lg">
        ...
        >
          <Tooltip title={"Make a new transfer"}>
            <Link
              to={routePaths.STOCKTRANSFORM + 0}
              type="submit"
            ...
            >
              <InteractionOutlined className="text-sm" />{" "}
              {"Make a new transfer"}
            </Link>
          </Tooltip>
        </motion.div>
              ...
              <tbody>
                {po_data &&
                  po_data?.slice(first, rows + first).map((item) => (
                    <tr
                      onClick={() => {
                        navigate(routePaths.STOCKTRANSFORM + item.sl_no);
                      }}
                    ...
                    >
                      ...
                      <td class="px-6 py-4 text-gray-600 text-wrap">{item.proj_name} ({item.proj_id})</td>
                      ...
                      <td class="px-3 py-4 flex gap-3">
                        <Link to={routePaths.STOCKTRANSFORM + item.sl_no}>
